...
const AboutSection = () => {
  ...
  const stats = [
    { icon: <Users className="w-10 h-10" />, number: "1,000+", label: "Satisfied Customers", description: "Trusted by mechanics nationwide", color: "from-blue-500 to-blue-600" },
    { icon: <Truck className="w-10 h-10" />, number: "5,000+", label: "Parts Shipped", description: "Successfully delivered components", color: "from-green-500 to-green-600" },
    { icon: <Shield className="w-10 h-10" />, number: "99.8%", label: "Quality Assurance", description: "Rigorous inspection standards", color: "from-purple-500 to-purple-600" },
    { icon: <BadgeDollarSign className="w-10 h-10" />, number: "40-70%", label: "Cost Savings", description: "Below dealer prices", color: "from-orange-500 to-orange-600" },
  ];

  const values = [
    { icon: <Wrench className="w-8 h-8" />, title: "Premium Quality", description: "Every component undergoes our comprehensive 12-point inspection process to ensure OEM-level performance and reliability.", features: ["12-point inspection", "OEM quality standards", "Performance tested"], color: "bg-blue-50 dark:bg-blue-900/20", iconColor: "text-blue-600 dark:text-blue-400" },
    { icon: <Timer className="w-8 h-8" />, title: "Fast Delivery", description: "Same-day shipping on orders placed before 3PM EST, with nationwide coverage and tracking on every shipment.", features: ["Same-day shipping", "Nationwide coverage", "Full tracking"], color: "bg-green-50 dark:bg-green-900/20", iconColor: "text-green-600 dark:text-green-400" },
    { icon: <DollarSign className="w-8 h-8" />, title: "Unbeatable Value", description: "Premium used parts at 40-70% below dealer prices, without compromising on quality or performance.", features: ["40-70% savings", "No quality compromise", "Transparent pricing"], color: "bg-purple-50 dark:bg-purple-900/20", iconColor: "text-purple-600 dark:text-purple-400" },
    { icon: <ThumbsUp className="w-8 h-8" />, title: "Expert Support", description: "ASE-certified technical staff available to help you find the right part and provide installation guidance.", features: ["ASE-certified staff", "Technical support", "Installation help"], color: "bg-orange-50 dark:bg-orange-900/20", iconColor: "text-orange-600 dark:text-orange-400" },
  ];
    ...
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="relative bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
          <p className="inline-block bg-white/10 border border-white/20 rounded-full px-4 py-1.5 text-orange-300 text-sm mb-6">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link
              href="/aboutus"
              className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-4 rounded-lg font-semibold transition-all duration-200 transform hover:scale-105"
            >
              Explore More
            </Link>
            <button
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, index) => (
              <div key={index} className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 text-center transform hover:scale-105 transition-all duration-300">
                <div className={`w-16 h-16 bg-gradient-to-r ${stat.color} rounded-full flex items-center justify-center mx-auto mb-4 text-white`}>
                  {stat.icon}
                </div>
                <div className="text-2xl lg:text-4xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-2">{stat.number}</div>
                <div className="text-xl lg:text-2xl sm:text-xl font-semibold text-gray-700 dark:text-gray-300 mb-1">{stat.label}</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
          <div className="relative">
            ...
            <div className="relative bg-gray-200 dark:bg-gray-700 rounded-2xl h-96 overflow-hidden">
              <Image src="/about/warehouse.png" alt="Spikey Salvage yard" fill className="object-cover rounded-2xl shadow-lg" />
            </div>
          </div>
          <div className="flex flex-col justify-center">
            ...
            <div className="bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 border-l-4 border-orange-500 p-6 rounded-lg">
              ...
              <p className="text-sm lg:text-lg md:text-base text-orange-900 dark:text-orange-200 font-semibold">
        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 gap-4 md:gap-8">
            {values.map((value, index) => (
              <div key={index} className={`${value.color} rounded-2xl p-4 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col`}>
                <div className="flex items-center mb-4 md:mb-6">
                  <div className={`w-10 h-10 md:w-16 md:h-16 ${value.iconColor} bg-white dark:bg-gray-800 rounded-full flex items-center justify-center mr-4 md:mr-6 shadow-md`}>
                    {value.icon}
                  </div>
                  <h3 className="text-lg md:text-2xl font-bold text-gray-900 dark:text-white">{value.title}</h3>
                </div>
                <p className="text-xs md:text-base text-gray-700 dark:text-gray-300 mb-4 md:mb-6 leading-relaxed">{value.description}</p>
                <div className="flex flex-wrap gap-1 md:gap-2">
                  {value.features.map((feature, idx) => (
                    <span key={idx} className="bg-white dark:bg-gray-800 px-2 md:px-3 py-0.5 md:py-1 rounded-full text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300 shadow-sm">
                      {feature}
                    </span>
                  ))}
                </div>
        ...
        <div className="bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-2xl p-6 md:p-10 lg:p-12 mb-16 md:mb-24">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
            {features.map((feature, index) => (
              <div key={index} className="bg-white dark:bg-gray-800 rounded-xl p-3 md:p-6 shadow-md hover:shadow-lg transition-all duration-300 flex flex-col justify-between">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-xs md:text-sm">{feature.text}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 rounded-2xl p-6 sm:p-8 md:p-12 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-4 w-full">
              <button
                onClick={() => setModalOpen(true)}
                className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-primary-foreground px-6 sm:px-10 py-3 sm:py-4 rounded-lg font-semibold transition-all duration-200 transform hover:scale-105 shadow-lg text-base sm:text-lg"
              >
                Shop Parts Now
              </button>
              <Link
                href="/contact"
                className="w-full sm:w-auto bg-transparent border-2 border-white text-white hover:bg-white hover:text-gray-900 px-6 sm:px-10 py-3 sm:py-4 rounded-lg font-semibold transition-all duration-200 shadow-lg text-base sm:text-lg mt-2 sm:mt-0 text-center"
              >
                Contact Our Experts
              </Link>
            </div>
